...
seneca.client({ prot: "5010", pin: { role: "coupons-store" } });
...
app.post('/submit', multipart, function (httpRequest, httpResponse, next) {
    ...
    fs.rename(tmp_path, renamed_path, function (err) {
        if (err) return httpResponse.status(500).send("An error accured");
            ...
            var req = request.post(storage_server_url.value + "/store", function (err, resp, body) {
                ...
                if (body == "Done") {
                    ...
                    seneca.act({
                        ...
                        desc: httpRequest.body.desc,
                        price: httpRequest.body.price,
                        discount: httpRequest.body.discount,
                        thumbnail_id: thumbnail_id + thumbnail_extension
